...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated == True:
            explosion_index = (self.age % 64) // 1
            self.image_center = [self.image_center[0] + explosion_index * self.image_size[0], self.image_center[1]]
        canvas.draw_image(self.image, self.image_center, self.image_size,
                          self.pos, self.image_size, self.angle)
...
# helper funcions
def process_sprite_group(set_x, canvas):
    for i in list(set_x):
        if not i.update():
            i.draw(canvas)
            i.update()
...
rock_group = set([])
#a_rock = Sprite([WIDTH / 3, HEIGHT / 3], [1, 1], 0, .1, asteroid_image, asterck_groupoid_info)
