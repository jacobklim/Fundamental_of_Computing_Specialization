...
def mm_move(board, player):
    ...
    for empty_square in board.get_empty_squares():
        ...
        cloned_board.move(empty_square[0], empty_square[1], player)
        score, dummy_move = mm_move(cloned_board, provided.switch_player(player))
        #best choce found
        if score * SCORES[player] == 1:
            return score, empty_square
        #update the initial
        elif score * SCORES[player] > result[0]:
            result = (score, empty_square)
        elif result[0] == -1:
            result = (result[0], empty_square)
